...
from llpsi_html.render import render_project
from llpsi_html.source import source_body
from llpsi_html.verify import verify_project
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"verify", "release"}:
        report = verify(root)
        print(json.dumps({"verified": str(root / "reports/verify.json"), "failures": len(report["failures"])}, indent=2))
